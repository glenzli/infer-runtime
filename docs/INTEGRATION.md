...
“查看模型组”会显式请求一次新清单（operator API 可用 `?fresh=1`）。一次成功清单
中缺席的已配置 Deployment 会立即退出新请求候选，但不会改变静态准入配置，也不会
使同一 Provider 下其他模型熔断。连续两次、相隔至少一小时的完整清单缺席会在
Console 标为“上游持续缺席”；读取清单失败只标记观测错误，不构成退役证据。执行
