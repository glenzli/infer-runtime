...
纵向切片，却无法区分实例、daemon generation、lease 或动态端口，也使每个租户各自维护一套
“服务是否存在”的猜测逻辑。Runtime 已按 `infra.discovery.registration@20260810.1` 发布只读
...
4. 显式 endpoint override 仍用于开发和诊断；租户迁移期可保留固定地址 fallback。稳定接入以
   Discovery 的 lease 和 generation 为准。
...
- `infer-runtime.status` 与 `infer-runtime.consumer` 共享 service generation，不共享 application
  framing、权限或数据面。
- 当前 registration publisher 随 observer 开关启动；没有真实部署需求前不增加第二套 lease 或
  新配置层。
